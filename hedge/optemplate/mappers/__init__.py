...
class OperatorReducerMixin(LocalOpReducerMixin, FluxOpReducerMixin):
    ...
    map_quadrature_grid_upsampler = map_diff_base
...
class IdentityMapperMixin(LocalOpReducerMixin, FluxOpReducerMixin):
    ...
    map_quadrature_grid_upsampler = map_elementwise_linear
...
class StringifyMapper(pymbolic.mapper.stringifier.StringifyMapper):
    ...
    def map_quad_stiffness_t(self, expr, enclosing_prec):
        return "StiffT%dq[%s]" % (
                expr.xyz_axis, expr.quadrature_tag)
    ...
    def map_quad_mass(self, expr, enclosing_prec):
        return "Mq[%s]" % expr.quadrature_tag
...
class QuadratureOperatorSpecializer(CSECachingMapperMixin, IdentityMapper):
    """Guided by a typedict, substitutes QuadratureSomethingOperators for
    plain SomethingOperators.
    """
    ...
    def map_operator_binding(self, expr):
        ...
        from hedge.optemplate.operators import (
                ...
                FluxOperator, QuadratureFluxOperator,
                BoundarizeOperator, QuadratureBoundarizeOperator,
                OperatorBinding)
        ...
        if isinstance(expr.op, MassOperator) and has_quad_operand:
            ...
            if (isinstance(expr.field, OperatorBinding)
                ...
                raise NotImplementedError(
                        "QuadratureGridUpsampler cannot yet be applied to "
                        "boundary operands that don't come directly out of "
                        "a BoundarizeOperator()")
        ...
        elif isinstance(expr.op, FluxOperator):
            ...
            def process_flux_arg(flux_arg):
                ...
                if repr_tag[0] is None:
                    repr_tag[0] = arg_repr_tag
                else:
                    if arg_repr_tag != repr_tag[0]:
                        raise TypeError("all arguments of a flux operator "
                                "must have the same representation tag, but I "
            ...
            if isinstance(repr_tag, QuadratureRepresentation):
                return QuadratureFluxOperator(
                        expr.op.flux, repr_tag.quadrature_tag) \
                                (self.rec(expr.field))
            else:
                return expr.op(self.rec(expr.field))
